...
export function renderTimer(main: HTMLElement) {
  ...
  function paint() {
    ...
    if (!focus) focus = ts[0];
    if (!focus) { big.innerHTML = sevenSeg('00:00', { height: 84 }); big.classList.remove('done'); lab.textContent = 'no timer'; sub.textContent = ''; arc.setAttribute('stroke-dashoffset', String(C)); ctl.innerHTML = ''; }
    else {
