...
export function renderTimer(main: HTMLElement) {
  ...
  main.append(html`
    <div class="ring">
      <svg class="dial" viewBox="0 0 268 268">${TICKS}<g transform="rotate(-90 134 134)"><circle cx="134" cy="134" r="${R}" fill="none" stroke="var(--line-soft)" stroke-width="13"/><circle id="arc" cx="134" cy="134" r="${R}" fill="none" stroke="var(--orange)" stroke-width="13" stroke-linecap="butt" stroke-dasharray="${C}" stroke-dashoffset="${C}"/></g></svg>
  ...
  function paint() {
    ...
    if (!focus) focus = ts[0];
    if (!focus) { big.innerHTML = sevenSeg('00:00', { height: 84 }); big.classList.remove('done'); lab.textContent = 'no timer'; sub.textContent = ''; arc.setAttribute('stroke-dashoffset', String(C)); arc.setAttribute('stroke', 'var(--orange)'); ctl.innerHTML = ''; }
    else {
      const rem = timerEngine.remaining(focus);
      big.innerHTML = sevenSeg(mmss(rem), { height: rem >= 3600000 ? 56 : 84 }); big.classList.toggle('done', focus.done);
      ...
      // the band is what is left; it turns red for the last 10 % (or 15 s), grey while paused
      const frac = focus.done ? 0 : rem / focus.durationMs;
      arc.setAttribute('stroke-dashoffset', String(C * (1 - frac)));
      arc.setAttribute('stroke', !focus.endAt && !focus.done ? 'var(--muted)' : rem <= Math.max(15e3, focus.durationMs * 0.1) ? 'var(--danger)' : 'var(--orange)');
