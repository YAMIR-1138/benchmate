...
// Short codes: samples are numbered, genes are lettered. They print in the wells and survive a black-and-white copy.
const sCode = (i: number) => String(i + 1);
const gCode = (j: number) => (j < 26 ? String.fromCharCode(97 + j) : `g${j + 1}`);
const codeBadge = (c: string) => `<span class="wcode">${c}</span>`;
